...
$(function(){
  ...
  $('li.dropdown').on('click', function(e){
    var navItem = $(this)[0];
    if( $(navItem).find('.open') ){
       var navTarget = $(navItem).find('a');
       var navLink = $(navTarget).attr('href');
       $(navTarget).attr('data-target',navLink);
...
var canvasNav,
  ...
  loadCanvasNav = function(){
    canvasNav = $siteNavContainer.scotchPanel({
        ...
        beforePanelOpen: function() {
          ...
          $siteNavContainer.animate({paddingTop:$('body').scrollTop() + 50});
        },
  ...
  removeCanvasNav = function(){
    $('.siteNav-container').removeAttr('style');
  ...
  $( window ).resize(function() {
     ...
     if ($(window).width() != windowWidth ) {
      ...
      windowWidth = $(window).width();
        ...
        if ( $(window).width() >= 1024 && isCanvasNavLoaded){
          removeCanvasNav();
        }
