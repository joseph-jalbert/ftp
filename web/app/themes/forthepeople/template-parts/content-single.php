...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<div itemscope itemtype="http://schema.org/Article" class="entry-content">
		...
		<h1 itemprop="headline" class="pagetitle"><?php echo get_the_title() ?></h1>
		<meta itemprop="publisher" content="Morgan &amp; Morgan" />
		<div class="subtitle"><?php the_field( 'sub_title' ); ?></div>
		<div class="heading-hr"></div><?php

		if ( ! in_category( 'Blog' ) && 'local_news' !== $post->post_type ) :
		...
		if ( get_field( 'practice_area_page' ) ) :
			include( 'all-practice-areas.php' );
		else :
			if ( get_field( 'jump_box' ) ) :
				?><div class="well pull-right span4 offset1">
					<?php the_field( 'jump_box' ); ?>
				</div><?php
			endif;
			if ( is_singular( 'post' ) || is_singular( Local_News::POST_TYPE ) || is_singular( BTG_News::POST_TYPE ) ) :
				?><div class="blog-post post-page"><?php
				...
				elseif ( get_field( 'imported_blog_post_image' ) ) :
					?><div class="title img">
				...
				?><div id="blogmeta" class="span3 push1 pull-left">
					<ul class="unstyled">
						...
						<li itemprop="author" itemscope itemtype="http://schema.org/Person"><i class="icon-user"></i> <span itemprop="name"><?php the_author(); ?> </span></li><?php
